 ...
 *         Worksheet 4: Exercise 4: Solution: LINES 133 - 168
 ...
 */
   ...
   /**
    * METHOD for generic bankaccount objects, it only returns the balance.
    * However, it is allowed to be overridden by the BankAccountWithOverdraft
    * account, which adds the value of overDraftLimit to the balance. This way
    * only overdraft bank accounts are allowed to go into the negatives by not
    * more than the limit set as the overDraftLimit.
    * @return balance
    */
    ...
    *  OR in case of a BankAccountWithOverdraft method, it ensures that the
    *  balance stays above the overdraft limit (in the negatives).
    *  @param amount The amount to be withdrawn.
    *  @param password The password of the account
    */
   public final void withdraw(long amount, String password) {
       if (checkPassword(password) && maxWithdrawable() >= amount){
           setBalance(getBalance() - amount);
       } else {
