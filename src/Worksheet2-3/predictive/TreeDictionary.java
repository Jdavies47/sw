...
/**
 * WS 2-3 PART 4
 * predictive.TreeDictionary creates a datastructure reading the words
 ...
 * that is called every other time. All predictive.TreeDictionary
...
public class TreeDictionary implements Dictionary {
    ...
    public TreeDictionary(String filepath) {
        ...
        try { // IF THE FILE EXISTS
            Scanner file = new Scanner(new FileReader(filepath)); // DECLARE A SCANNER
            while (file.hasNextLine()) { // WHILE THERE ARE WORDS IN THE DATABASE
                String word = file.nextLine().toLowerCase(); // STORE THEM FOR EACH LOOP
                if (isValidWord(word)) { // IF THEY'RE VALID WORDS
                    String sign = wordToSignature(word); // CREATE THEIR SIGNATURE VALUES
                    addWordToTree(word, sign, 0); // SEND DATA TO THE METHOD THAT ADDS WORD TO THE TREE
                }
            }
            file.close();
    ...
    /**
     * addWordToTree is called by the predictive.TreeDictionary(String filepath) constructor.
    ...
    public Set<String> signatureToWords(String signature) {
        ...
        // CREATE AN ARRAY OF keysPressed THAT'S THE SAME LENGTH AS THE SIGNATURE'S LENGTH
