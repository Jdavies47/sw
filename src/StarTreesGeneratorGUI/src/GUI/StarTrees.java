...
public class StarTrees extends BaubleTrees {
    ...
    int colourOfBaubles, colourOfStar, starThickness;
    /**
     * VARIABLES:
     ...
     * int userSteps to store the amount of steps taken to arrive to one
     * point from another when drawing its lines
     */
    private ArrayList<Polygon> toDraw = new ArrayList<>();
    private int numberOfBaubles, numberOfVertices, userSteps;
     ...
     *                         of the star on top of the christmas tree
     * @param userSteps        the amount of steps taken to arrive to one
     *                         point from another when drawing its lines
     */
    public StarTrees(int[] xTrees, int[] yTrees, int[] scaleTrees, int numberOfBaubles, int numberOfVertices, int userSteps) {
    ...
    public static void main(String[] args) {
        ...
        StarTrees christmas = new StarTrees(xTrees, yTrees, scaleTrees, numberOfBaubles, numberOfVertices, userSteps);
...
//        SNOWING EFFECT
//        snowFall letItSnow = new snowFall(2, 10);
//        letItSnow.setVisible(true);
//        letItSnow.setSize(FRAME_WIDTH, FRAME_HEIGHT);
//        letItSnow.setBackground(new Color(0.0001f, 0.0001f, 0.0001f, 0.0001f));
...
// DRAWING THE BAUBLES BEGINS HERE---------------------------------------------
        // LOOPS THROUGH THE toDraw LIST ALL ITS MEMBERS
        for (int i = 1; i < toDraw.size(); i++) {
            ...
            if (toDraw.get(i).npoints == 3) {
                ...
                for (int j = 0; j < numberOfBaubles; j++) {
                    ...
                    if (colourOfBaubles == 3) { // if random colouring is selected
                    ...
                    } else if (colourOfBaubles == 0) { // user selects "red"
                        Color niceRed = new Color(208, 14, 14);
                        g.setColor(niceRed);
                    } else if (colourOfBaubles == 1) { // user selects "blue"
                        Color niceBlue = new Color(34, 97, 181);
                        g.setColor(niceBlue);
                    } else if (colourOfBaubles == 2) { // user selects "yellow"
                        Color niceYellow = new Color(225, 201, 34);
                        g.setColor(niceYellow);
                    }
...
// DRAWING THE STAR BEGINS HERE------------------------------------------------
                ...
                if (colourOfStar == 10) colourOfStar = randomInt;
                ...
                for (int j = 0; j < numberOfVertices; j++) {
                    ...
                    if (j < numberOfVertices - userSteps) {
                        g2.setStroke(new BasicStroke(starThickness));
                        g2.drawLine(starXpoints[i][j], starYpoints[i][j], starXpoints[i][j + userSteps],
                                starYpoints[i][j + userSteps]);
                    } else {
                        // temporary value, so that j remains unchanged
                        int temp = j;
                        temp = temp - (numberOfVertices - userSteps);
                        // Star is drawn
                        g2.setStroke(new BasicStroke(starThickness));
                        g2.drawLine(starXpoints[i][j], starYpoints[i][j], starXpoints[i][temp], starYpoints[i][temp]);
                    }
                }
            }
        }
